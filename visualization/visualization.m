...
fname      = '/Users/wanqiaod/workspace/pipeline/test_video/LeBlanc_short.mp4';

[~,fname_char,~] = fileparts(fname);
norm_dir   = '/Users/wanqiaod/workspace/pipeline/out/feta_out/feta_norm_videos/';
AU_dir     = '/Users/wanqiaod/workspace/pipeline/out/AU_detector_out/';
norm_fname = fullfile(norm_dir,[fname_char '_norm.mp4']);
AU_fname   = fullfile(AU_dir,[fname_char '_au_out.mat']);

v1  = VideoReader(norm_fname);
aus = [1,2,4,6,7,10,12,14,15,17,23,24];

load(AU_fname);
frame_cnt = floor(v1.Duration * v1.FrameRate);
y_pred    = table2array(result);
out_fname = [fname_char '_out_video.mp4'];

% y_pred contains AU probability outputs of the pipeline
v = VideoWriter(out_fname);
...
limit1 = 1;
limit2 = frame_cnt; % number of frames to be visualized (can be v1.NumberOfFrames);
