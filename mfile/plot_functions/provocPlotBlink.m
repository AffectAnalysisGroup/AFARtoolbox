...
temp_avg_dist = blink.avg_dist;
temp_interval = blink.blink_interval;
if start_frame > half_seg_time_width
    padded_avg_dist = temp_avg_dist((start_frame-half_seg_time_width):total_frame,1);
    padded_interval = temp_interval((start_frame-half_seg_time_width):total_frame,1);
else
    padded_avg_dist = temp_avg_dist(1:total_frame,1);
    padded_interval = temp_interval(1:total_frame,1);
    padded_zero_num = half_seg_time_width - start_frame;
    padded_zero     = zeros([padded_zero_num 1]);
    padded_avg_dist = [padded_zero padded_avg_dist];
    padded_interval = [padded_zero padded_interval];
end

if end_frame < total_frame-half_seg_time_width
    padded_avg_dist = padded_avg_dist(1:(end_frame-start_frame+seg_time_width));
    padded_interval = padded_interval(1:(end_frame-start_frame+seg_time_width));
else
    padded_zero_num = end_frame + half_seg_time_width - total_frame;
    padded_zero     = zeros([padded_zeros_num 1]);
    padded_avg_dist = [padded_avg_dist padded_zero];
    padded_interval = [padded_interval padded_zero];
end
...
for frame_index = start_frame : end_frame
    ...
    seg_time_x     = frame_index : frame_index + seg_time_width - 1;
    seg_timeline_x = frame_index;
    interval_seg   = padded_interval(seg_time_x);
    avg_dist_seg   = padded_avg_dist(seg_time_x);
    seg_plot_x_ax  = (frame_index - half_seg_time_width) : (frame_index + ...
                      half_seg_time_width - 1);
